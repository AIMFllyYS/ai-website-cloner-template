...
license: Apache-2.0
compatibility: Requires Node.js >= 24, a shell, network access, and a browser-automation capability (any MCP/driver). Outputs a Next.js project.
metadata:
  author: JCodesMore (original engine), 羽升/AIMFllyYS (Stage-2 integration)
  version: "1.1.0"
---

# Clone Website

> **Platform · any spec-compliant agent (portable).** Install into your agent's skills directory. Activate however your agent triggers skills (description match, slash command, or by name). Phrased tool-agnostically — "run X" means execute it in the shell; "edit a file" means use your agent's file primitive. Requires a **browser-automation capability** (any MCP/driver) plus network access for downloads. Parallel builders use your agent's subagent/parallel mechanism, or build sections sequentially.
