...
license: Apache-2.0
compatibility: Requires a shell with bash, Node.js (local server + capture script), wget or curl, and network access to the target site. Stage 2 additionally requires Node.js >= 24 and a browser-automation capability. Stage 1 runs on any agent with shell access.
metadata:
  author: 羽升 (AIMFllyYS)
  version: "1.1.0"
---
...
> **平台 · 任意合规 Agent（可移植）.** 安装到你的 agent 技能目录（如 `~/.agents/skills/`、`.claude/skills/` 或其它）。按你的 agent 的方式激活（描述匹配 / 斜杠命令 / 按名）。本变体用工具无关措辞编写：“读取/编辑文件” = 用你 agent 的文件原语；“运行 …” = 在 shell 执行。需求：shell、Node.js、`wget`/`curl`、对目标站的**网络访问**（部分 agent 会沙箱网络，需手动开启）。Stage 2 另需 Node.js ≥ 24 与浏览器自动化能力；并行重建用你 agent 的子代理/并行机制，或顺序逐区进行。
