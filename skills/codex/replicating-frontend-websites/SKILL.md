...
description: Use when asked to clone, mirror, archive, scrape, or 1:1 replicate an existing website's frontend (复刻/克隆/镜像/抓站/扒站/离线备份/还原网站/搬运网站), including static sites, SPAs, SSG/SSR sites, and WebGL/Three.js experience sites. Use even if the user only gives a URL and says "copy this site", "把这个网站搬下来", or asks how to make an offline copy of a web page's design. Do NOT trigger for building a brand-new site from scratch, backend/API work, or non-web tasks.
---

# 1:1 复刻前端网站（通解）

> **平台 · OpenAI Codex.** 安装到 `~/.agents/skills/replicating-frontend-websites/`（全局）或 `<repo>/.agents/skills/`（仓库级）。用 `/skills` 或 `$replicating-frontend-websites` 触发。Codex 通过 **shell + `apply_patch`** 工作（没有独立的 Read/Edit/Write 工具）——本技能凡提到某个工具处，直接跑等价 shell 命令即可。**网络默认关闭**：抓取/下载用到 `wget`/`curl`/`node`/`npm`，请让 Codex 开启网络（`~/.codex/config.toml` 里 `network_access = true`、或逐次批准、或 Full Access）。Stage 2 的浏览器捕获需在 Codex 里配置浏览器自动化 MCP（没有 `claude --chrome` 开关）。并行重建 = Codex 子代理（`/agent`）+ git worktree。
