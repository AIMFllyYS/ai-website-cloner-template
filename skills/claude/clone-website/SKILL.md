...
> **Platform · Claude Code.** Install to `.claude/skills/clone-website/`. Invoke with `/clone-website <url>` (or it auto-activates from the description). Requires a browser MCP — launch with `claude --chrome`. Parallel builders run via the Task/subagent tool in git worktrees.
